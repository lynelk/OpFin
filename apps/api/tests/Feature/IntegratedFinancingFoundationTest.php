...
class IntegratedFinancingFoundationTest extends TestCase
{
    ...
    public function test_sharia_only_intent_never_returns_conventional_product(): void
    {
        ...
        $approval = ShariaApproval::create([
            ...
            'authority_name' => 'Test Sharia Authority',
            'approval_reference' => 'TEST-SHARIA-1',
            'scope_type' => 'product',
            'scope_reference' => 'MUR-1',
            'status' => 'approved',
        ...
        $intent = $this->postJson('/api/financial-intents', [
            'financial_space_id' => $space,
            'need_type' => 'merchant_inventory',
            'principles_preference' => 'SHARIA_ONLY',
            'amount_minor' => 100000,
        ...
        $this->postJson('/api/product-matches', ['financial_intent_id' => $intent['id']])
            ...
            ->assertJsonPath('data.0.id', $islamic->id);
    }
    ...
    public function test_expired_sharia_approval_blocks_islamic_product(): void
    {
        ...
        $approval = ShariaApproval::create([
            ...
            'approval_reference' => 'EXPIRED-1',
            'scope_type' => 'product',
            'scope_reference' => 'MUR-X',
            'status' => 'approved',
    ...
    public function test_financial_space_isolation_is_enforced(): void
    {
        ...
        $this->postJson('/api/financial-intents', [
            'financial_space_id' => $space,
            'need_type' => 'cashflow',
        ])->assertUnprocessable();
    }
    ...
    private function passport(string $status = 'approved'): LegalProductPassport
    {
        return LegalProductPassport::create([
            'reference' => (string) Str::uuid(),
            'jurisdiction' => 'UG',
            'regulated_activity' => 'financing',
            'booking_entity' => 'Test regulated partner',
            'status' => $status,
            'effective_from' => now()->subDay(),
    ...
    private function product(string $code, string $rail, int $passportId, ?int $shariaId): FinancialProduct
    {
        return FinancialProduct::create([
            ...
            'rail' => $rail,
            'family' => 'finance',
            'contract_type' => $rail === 'ISLAMIC' ? 'MURABAHA' : 'CREDIT',
            'jurisdiction' => 'UG',
            'currency' => 'UGX',
            'legal_product_passport_id' => $passportId,
            'sharia_approval_id' => $shariaId,
            'status' => 'live',
            'effective_from' => now()->subDay(),
        ]);
    }
