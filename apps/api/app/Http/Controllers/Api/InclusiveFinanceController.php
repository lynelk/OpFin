...
class InclusiveFinanceController extends Controller
{
    ...
    private function programmeValidation(Request $request, bool $partial = false): array
    {
        ...
        return $request->validate([
            ...
            'eligibility_rules.all' => ['sometimes', 'array'],
            'eligibility_rules.all.*.field' => ['required_with:eligibility_rules.all', Rule::in(InclusiveFinanceService::PROGRAMME_ELIGIBILITY_FIELDS)],
            'eligibility_rules.all.*.operator' => ['required_with:eligibility_rules.all', Rule::in(['equals', 'in'])],
            'eligibility_rules.all.*.value' => ['nullable'],
            'eligibility_rules.all.*.values' => ['sometimes', 'array'],
            'eligibility_rules.any' => ['sometimes', 'array'],
            'eligibility_rules.any.*.field' => ['required_with:eligibility_rules.any', Rule::in(InclusiveFinanceService::PROGRAMME_ELIGIBILITY_FIELDS)],
            'eligibility_rules.any.*.operator' => ['required_with:eligibility_rules.any', Rule::in(['equals', 'in'])],
            'eligibility_rules.any.*.value' => ['nullable'],
            'eligibility_rules.any.*.values' => ['sometimes', 'array'],
