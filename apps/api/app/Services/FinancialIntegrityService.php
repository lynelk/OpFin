...
class FinancialIntegrityService
{
    ...
    public function run(string $scope = 'platform'): object
    {
        ...
        if (Schema::hasTable('mobile_money_transactions')) {
            ...
            $legacyUnknownSubmissions = MobileMoneyTransaction::query()
                ->whereIn('status', [
                    MobileMoneyTransaction::STATUS_PROCESSING,
                    MobileMoneyTransaction::STATUS_PENDING,
                ])
                ->where('provider_submission_state', 'legacy_unknown')
                ->whereNull('provider_reference')
                ->count();
            if ($legacyUnknownSubmissions > 0) {
                $findings[] = $this->alert($runId, 'high', 'legacy_unknown_provider_submission', null,
                    'Migrated pending money movements have unknown provider-submission state and require provider lookup or statement reconciliation before financial readiness.', [
                        'count' => $legacyUnknownSubmissions,
                    ]);
            }
