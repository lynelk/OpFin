...
class CpayEssentialsClient
{
    public function lookup(EssentialsAccount $account): array
    {
        ...
        return $this->sendSigned($path, [
            ...
            'customerReference' => 'user:'.$account->user_id,
            'requestReference' => 'ESS-LOOKUP-'.$account->public_id,
        ]);
    }
    ...
    public function payBill(EssentialsAdvance $advance, EssentialsAccount $account, EssentialsBiller $biller): array
    {
        ...
        return $this->sendSigned($path, [
            ...
            'metadata' => [
                ...
                'lenderPartnerId' => $advance->lender_partner_id,
    ...
    public function collectRepayment(
        ...
        string $payerMsisdn,
        ?string $payerProvider = null,
    ): array {
        $path = $this->path('lender_repayment_path');

        return $this->sendSigned($path, [
            'merchantNumber' => $this->merchantNumber(),
            'country' => strtoupper((string) config('services.cpay.country', 'UG')),
            'currency' => $advance->currency,
            'amountMinor' => $repayment->amount_minor,
            'payerMsisdn' => $payerMsisdn,
            'payerProvider' => $payerProvider,
            'requestReference' => $repayment->reference,
            'purpose' => 'opfin_essentials_repayment',
    ...
    {
        $path = $this->path('transaction_status_path');
    ...
    {
        $pathKey = $biller->route === 'manual_verification' ? 'beneficiary_payment_path' : 'bill_payment_path';

        return $this->configured($pathKey);
    }
    ...
    private function path(string $key): string
    {
        if (! $this->configured($key)) {
            throw new RuntimeException("CPay Essentials capability {$key} is not configured.");
        }

        $path = trim((string) config('services.cpay.'.$key));
    ...
    private function sendSigned(string $path, array $payload): array
    {
        ...
        $response = Http::withHeaders([
            ...
            ->send('POST', rtrim((string) config('services.cpay.base_url'), '/').$path);
    ...
    private function decode(Response $response): array
    {
        if (! $response->successful()) {
            $code = (string) ($response->json('code') ?? 'CPAY_ESSENTIALS_ERROR');
            throw new RuntimeException("CPay Essentials request failed with {$code} (HTTP {$response->status()}).");
        }
