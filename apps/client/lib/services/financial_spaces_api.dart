...
class FinancialSpacesApi {
  ...
  static Future<List<Map<String,dynamic>>> treasuryAccounts(int id) async => (((await _request('/financial-spaces/$id/treasury/accounts'))['accounts'] as List?)??[]).whereType<Map>().map((e)=>e.cast<String,dynamic>()).toList();
  static Future<List<Map<String,dynamic>>> generatedStatements(int id,{int? accountId}) async => (((await _request('/financial-spaces/$id/statements' + (accountId==null?'':'?account_id=$accountId')))['statements'] as List?)??[]).whereType<Map>().map((e)=>e.cast<String,dynamic>()).toList();
  static Future<Map<String,dynamic>> issueStatement(int id,int accountId,String from,String to) async => ((await _request('/financial-spaces/$id/treasury/accounts/$accountId/statements',method:'POST',body:{'from':from,'to':to}))['statement'] as Map).cast<String,dynamic>();
  static Future<Map<String,dynamic>> issueConsolidatedStatement(int id,String from,String to) async => ((await _request('/financial-spaces/$id/statements/consolidated',method:'POST',body:{'from':from,'to':to}))['statement'] as Map).cast<String,dynamic>();
  static Future<Map<String,dynamic>> generatedStatement(int id,int statementId) async => (await _request('/financial-spaces/$id/statements/$statementId')).cast<String,dynamic>();
  static Future<List<Map<String,dynamic>>> assets(int id) async => (((await _request('/financial-spaces/$id/assets'))['assets'] as List?)??[]).whereType<Map>().map((e)=>e.cast<String,dynamic>()).toList();
