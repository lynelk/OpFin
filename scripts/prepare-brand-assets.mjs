...
if (!hasTransparency) {
  // The original raster includes a flat background. Flood-fill only matching
  // pixels connected to the border, preserving enclosed white artwork.
  ...
  function enqueue(index) {
    ...
    seen[index] = 1;
    const p = index * 4;
    if (Math.max(...cornerColour.map((value, channel) => Math.abs(data[p + channel] - value))) <= 24) queue[tail++] = index;
  }
  ...
  while (head < tail) {
    ...
    enqueue(index - info.width); enqueue(index + info.width);
  }
}
let left = info.width, top = info.height, right = -1, bottom = -1, visible = 0;
for (let y = 0; y < info.height; y++) for (let x = 0; x < info.width; x++) {
  const p = (y * info.width + x) * 4;
  if (data[p + 3] > 16) { visible++; left = Math.min(left, x); top = Math.min(top, y); right = Math.max(right, x); bottom = Math.max(bottom, y); }
}
...
manifest.extraction = { cornerColour, sourceHasTransparency: hasTransparency, bounds: { left, top, width, height } };
...
console.log('OPFIN_LOGO_REVIEW_PNG=' + (await sharp(icon).resize(128, 128).png().toBuffer()).toString('base64'));
